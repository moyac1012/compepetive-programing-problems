...
struct UnionFind {
    ...
    UnionFind(int size) : n(size), parent(size + 1), diff_weight(size + 1, 0) {
    ...
    int find_set(int x) {
        ...
        diff_weight[x] += diff_weight[orig_parent];
        return parent[x];
    }
    ...
    bool union_set(int x, int y, ll w) {
        ...
        if(root_x == root_y){
            return (diff_weight[y] - diff_weight[x]) == w;
        }
